...
{% embed "snipplets/forms/form-select.tpl" with{select_label: false, select_custom_class: 'js-sort-by ordenar-txt', select_group_custom_class: 'mb-0', select_aria_label: 'Ordenar por:' | translate } %}
	{% block select_options %}
		{% for sort_method in sort_methods %}
	        ...
	        {% if sort_method != 'user' or category.sort_method == 'user' %}
	            <option class="ordenar-txt" value="{{ sort_method }}" {% if sort_by == sort_method %}selected{% endif %}>{{ sort_text[sort_method] | t }}</option>
	        {% endif %}
	    {% endfor %}
